...
export async function getGuildSharedPosts(lastCursor: string, take: number, guildId: string) {
    const res = await axios.get("/api/guild/shared-posts", { params: { guildId, lastCursor, take }})
    return res.data.data as TPage<TGuildSharedPost>
}

export function useGetGuildSharePost(guildId: string) {
...
export function useSharePost(){
    ...
    return useMutation({
        mutationFn: async ({ content, postId, guildId }: { content: string, guildId: string, postId: string}) => {
            return await axios.post("/api/share", { content }, { params: { postId, guildId }})
        },
...
export function useUpdateSharedPost(){
    ...
    return useMutation({
        mutationFn: async ({ content, shareId }: { content: string, shareId: string, guildId: string}) => {
            return await axios.patch("/api/share", { content }, { params: { shareId }})
        },
        onSuccess: async (data, { guildId }) => {
            await queryClient.invalidateQueries({queryKey: [QueryKeys.GuildSharedPosts, guildId]})
        },
